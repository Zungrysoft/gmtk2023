...
import { getUnlockedLevels, getLevelNumber } from './levelloader.js'
...
export default class LevelSelect extends Thing {
  ...
  constructor (previousMenu = TitleScreen) {
    super()
    this.menu = getUnlockedLevels()
    this.selection = this.menu.map(x => x.level).indexOf(game.globals.level)
  ...
  update () {
    ...
    if (this.selection !== lastSelection) {
      ...
      game.globals.level = this.menu[this.selection].level
      game.addThing(new Board())
      game.getThing('board').movementDisabled = true
      this.snapCamera()
    }
  ...
  postDraw () {
    ...
    for (let i = 0; i < this.menu.length; i += 1) {
      ...
      const checkbox = game.globals.levelCompletions[this.menu[i].level] ? 'checkbox_checked' : 'checkbox_unchecked'
      ...
      const levelName = `${getLevelNumber(this.menu[i].level)}: ${this.menu[i].name}`
      ctx.fillStyle = '#21235B'
